...
- **BREAKING: requires `marque/deck` `^2.0` instead of `marque/ise` `^1.0`.** The
  ...
  changes the install set — no disguise class, route or component changed.

- **disguise registers its own nav entry**, against trove's `NavRegistry`, with no
  visibility rule: disguise is the public frontend and guest browsing is the whole
  point, so the entry shows to everyone.
